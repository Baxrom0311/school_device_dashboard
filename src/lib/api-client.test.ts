...
// Install an in-memory localStorage shim before importing modules that
// transitively pull in zustand's persist middleware.
...
// Provide a minimal BroadcastChannel polyfill for the auth-store import chain.
...
describe('api-client', () => {
  ...
  it('attaches Bearer token to requests when access cookie is present', async () => {
  ...
  it('refreshes on 401 and retries the original request', async () => {
    ...
    expect(res.status).toBe(200)
    expect(calls).toBe(2)
    // Refresh endpoint must have been hit exactly once.
  ...
  it('clears auth and navigates when no refresh token is present on 401', async () => {
    ...
    expect(navigateTo).toHaveBeenCalledWith('/sign-in', { replace: true })
